...
// #4
void __real__ZN6CirBufC1Eli(CirBuf *cirBuf, long size, int unknown);
void __wrap__ZN6CirBufC1Eli(CirBuf *cirBuf, long size, int unknown)
{
    CameraBoost *cameraBoost = getCameraBoost(sOpeningCameraID);
    if (cameraBoost)
    {
    ...
    __real__ZN6CirBufC1Eli(cirBuf, size, unknown);
}
...
// CirBuf::ReadBuff
int __real__ZN6CirBuf8ReadBuffEPhiiPS0_(CirBuf *cirBuf, uchar* buffer, uint size, uint timeout);
int __wrap__ZN6CirBuf8ReadBuffEPhiiPS0_(CirBuf *cirBuf, uchar* buffer, uint size, uint timeout)
{
    CameraBoost *cameraBoost = getCameraBoost(cirBuf);
    if (cameraBoost)
        return cameraBoost->ReadBuff(buffer, size, timeout);
    else
        return __real__ZN6CirBuf8ReadBuffEPhiiPS0_(cirBuf, buffer, size, timeout);
}
